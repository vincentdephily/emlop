...
fn predict_emerge_p() {
    let t = vec![// Check garbage input
                 ...
                 ("[ebuild   R   ~] dev-qt/qtcore-5.9.4-r2\n\
                  ...
                  0),];
    for (i, o, e) in t {
        emlop("-F test/emerge.10000.log p --date unix").write_stdin(i).assert().code(e).stdout(o);
    }
}
...
fn stats_grouped() {
    let t: Vec<(&str, &str)> =
        vec![("-F test/emerge.10000.log s --duration s -sp gentoo-sources -gy",
             ...
             ("-F test/emerge.10000.log s --duration s -ss -gy", "2018 gentoo  150  4747  28\n"),
